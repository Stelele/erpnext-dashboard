<template>
    <div class="w-full h-full grid grid-cols-1 gap-4 p-4">
        <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
            ...
            <UFormField label="Expense Type" name="expenseType" :required="true"
                description="Please select the expense type.">
                <UInputMenu v-model="state.expenseType" :items="expenseTypes" class="w-full" />
            </UFormField>

            <UFormField label="Amount" name="amount" :required="true"
                description="Please enter the amount of the expense.">
                <UInputNumber v-model="state.amount" class="w-full" :step="0.01" />
            </UFormField>

            <UFormField label="Description" name="description" :required="true"
...
const schema = z.object({
    ...
    }).transform(({ year, month, day }) => shallowRef(new CalendarDate(year, month, day))),
...
const state = reactive<Schema>({
    date: shallowRef(new CalendarDate(moment().year(), moment().month() + 1, moment().date())),
    expenseType: 'Sekuru',
    amount: 0,
    description: '',
})
...
function onSubmit() {
    const expense: Expense = {
        date: moment(state.date.toDate(getLocalTimeZone())).format('YYYY-MM-DD'),
